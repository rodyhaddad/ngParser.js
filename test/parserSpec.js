...
describe('parser', function () {
  ...
  beforeEach(function () {
    // clear caches
    ngParser.getterFnCache = {};
    ngParser.promiseWarningCache = {};
      ...
      toThrowMinErr: function (namespace, code, content) {
        var result,
          ...
          regex = jasmine.isA_("RegExp", content) ? content :
            ngParser.isDefined(content) ? new RegExp(escapeRegexp(content)) : undefined;

        if (!ngParser.isFunction(this.actual)) {
  ...
  var scope, $rootScope, deferred, promise, q,
    $parse = ngParser,
  ...
  ngParser.forEach([true, false], function (cspEnabled) {
    ngParser.forEach([true, false], function (unwrapPromisesEnabled) {

      describe('csp: ' + cspEnabled + ", unwrapPromises: " + unwrapPromisesEnabled, function () {

        beforeEach(function () {
          ngParser.$parseOptions.unwrapPromises = unwrapPromisesEnabled;
          ngParser.$parseOptions.csp = cspEnabled;
        });
        ...
        it('should parse filters', function () {
          ngParser.registerFilter('substring', ngParser.valueFn(function (input, start, end) {
            return input.substring(start, end);
          }));

          expect(function () {
            scope.$eval("1|nonexistent");
          }).toThrowMinErr('$parse', 'unfil', 'Unknown filter: nonexistent');
        ...
        describe('sandboxing', function () {
          ...
          describe('Window and $element/node', function () {
            it('should NOT allow access to the Window or DOM when indexing', function () {
            ...
            it('should NOT allow access to the Window or DOM returned from a function', function () {
              scope.getWin = ngParser.valueFn($window);
              scope.getDoc = ngParser.valueFn($document);
            ...
            it('should NOT allow calling functions on Window or DOM', function () {
        ...
        describe('locals', function () {
          it('should expose local variables', function () {
            ...
            expect($parse('add(a,b)')({b: 1, add: function (a, b) {
              return a + b;
            }}, {a: 2})).toEqual(3);
          });
  ...
  describe('promises', function () {
    ...
    describe('unwrapPromises setting', function () {
      ...
      it('should not unwrap promises by default', function () {
    ...
    /*ngParser.forEach([true, false], function (cspEnabled) {
     ...
     beforeEach(function () {
     ngParser.$parseOptions.unwrapPromises = true;
     });
    ...
    ngParser.forEach([true, false], function (cspEnabled) {
      ...
      describe('csp ' + cspEnabled, function () {
        ...
        beforeEach(function () {
          ngParser.$parseOptions.unwrapPromises = true;
          ngParser.$parseOptions.logPromiseWarnings = false;
        });
