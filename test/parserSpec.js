...
describe('ngParser', function () {

    function ngParserEval(text, scope, locals) {
        return ngParser(text)(scope, locals);
    }

    it("should be available in the global scope", function () {
        expect(typeof ngParser).not.toBe("undefined");
    });

    describe('lexer', function () {
        it('should tokenize a string', function () {
            var tokens = ngParser.lex("a.bc[22]+1.3|f:'a\\\'c':\"d\\\"e\"");
            var i = 0;
        ...
        it('should tokenize undefined', function () {
            var tokens = ngParser.lex("undefined");
            var i = 0;
            expect(tokens[i].index).toEqual(0);
        ...
        it('should tokenize unicode', function () {
            var tokens = ngParser.lex('"\\u00A0"');
            expect(tokens.length).toEqual(1);
            expect(tokens[0].string).toEqual('\u00a0');
        });

        it('should ignore whitespace', function () {
            var tokens = ngParser.lex("a \t \n \r b");
        ...
        it('should tokenize function invocation', function () {
            var tokens = ngParser.lex("a()")
            expect(tokens.map(function (t) {
                return t.text;
            })).toEqual(['a', '(', ')']);
        });

        it('should tokenize method invocation', function () {
            var tokens = ngParser.lex("a.b.c (d) - e.f()");
            expect(tokens.map(function (t) {
                return t.text;
            })).
                toEqual(['a.b', '.', 'c', '(', 'd', ')', '-', 'e', '.', 'f', '(', ')']);
        ...
        it('should tokenize number', function () {
            var tokens = ngParser.lex("0.5");
            expect(tokens[0].text).toEqual(0.5);
        });

        it('should tokenize negative number', function () {
            var value = ngParserEval("-0.5");
            expect(value).toEqual(-0.5);

            value = ngParserEval("{a:-0.5}");
            expect(value).toEqual({a: -0.5});
        });

        it('should tokenize number with exponent', function () {
            var tokens = ngParser.lex("0.5E-10");
            expect(tokens[0].text).toEqual(0.5E-10);
            expect(ngParserEval("0.5E-10")).toEqual(0.5E-10);

            tokens = ngParser.lex("0.5E+10");
            expect(tokens[0].text).toEqual(0.5E+10);
        });
    ...
    ngParser.forEach([true, false], function (cspEnabled) {

        describe('csp ' + cspEnabled, function () {

            beforeEach(function ($sniffer) {
                scope = {
                    $eval: function (text) {
                        return ngParserEval(text, scope);
                    }
                };
                ngParser.csp = cspEnabled;
                ngParser.cache = {}; // clear cache
            });
            ...
            it('should parse ternary', function () {
                var returnTrue = scope.returnTrue = function () {
                    return true;
                };
                var returnFalse = scope.returnFalse = function () {
                    return false;
                };
                var returnString = scope.returnString = function () {
                    return 'asd';
                };
                var returnInt = scope.returnInt = function () {
                    return 123;
                };
                var identity = scope.identity = function (x) {
                    return x;
                };
            ...
            it('should parse filters', function () {
                ngParser.filters.register('substring', ngParser.valueFn(function (input, start, end) {
            ...
            it('should evaluate object', function () {
                expect(ngParser.toJson(scope.$eval("{}"))).toEqual("{}");
                expect(ngParser.toJson(scope.$eval("{a:'b'}"))).toEqual('{"a":"b"}');
                expect(ngParser.toJson(scope.$eval("{'a':'b'}"))).toEqual('{"a":"b"}');
                expect(ngParser.toJson(scope.$eval("{\"a\":'b'}"))).toEqual('{"a":"b"}');
            });
            ...
            it('should evaluate JSON', function () {
                expect(ngParser.toJson(scope.$eval("[{}]"))).toEqual("[{}]");
                expect(ngParser.toJson(scope.$eval("[{a:[]}, {b:1}]"))).toEqual('[{"a":[]},{"b":1}]');
            });
            ...
            it('should call the function once when it is part of the context on array lookup function', function () {
                var count = 0;
                var element = {name: {anotherFn: function () {
                    return 'lucas';
                } } };
                scope.fn = function () {
                    count++;
            ...
            describe('promises', function () {
                ...
                beforeEach(function () {
                    var resolveValue, thenResolve = [], thenReject = [], state = null, digestRun = false;
                        ...
                        defer: function () {
                            return {
                                ...
                                digest: function () {
                                    ...
                                    ngParser.forEach(fns, function (fn) {
                                        if (fn) {
                                            fn(resolveValue);
                                        }
                                    });
                                ...
                                promise: {
                                    then: function (resolve, reject) {
                                        thenResolve.push(resolve);
                                        thenReject.push(reject);
                                        if (digestRun && state) {
                                            setTimeout(function () {
                                                switch (state) {
                                                    case "resolve":
                                                        resolve && resolve(resolveValue);
                                                        break;
                                                    case "reject":
                ...
                describe('{{promise}}', function () {
                    ...
                    describe('assignment into promises', function () {
                        ...
                        it('should evaluate a resolved object promise and set its value', function () {
                            scope.person = promise;
                            deferred.resolve({'name': 'Bill Gates'});

                            var getter = ngParser('person.name');
                        ...
                        it('should evaluate a resolved primitive type promise and set its value', function () {
                            scope.greeting = promise;
                            deferred.resolve('Salut!');

                            var getter = ngParser('greeting');
                            expect(getter(scope)).toBe(undefined);
                        ...
                        it('should evaluate an unresolved promise and set and remember its value', function () {
                            scope.person = promise;
                            ...
                            var c1Getter = ngParser('person.A.B.C1');
                            scope.$digest();
                            ...
                            var c2Getter = ngParser('person.A.B.C2');
                            scope.$digest();
                            ...
                            expect(ngParser('person.A')(scope)).toEqual(
            ...
            describe('locals', function () {
                it('should expose local variables', function () {
                    expect(ngParser('a')({a: 0}, {a: 1})).toEqual(1);
                    expect(ngParser('add(a,b)')({b: 1, add: function (a, b) {
                        return a + b;
                    }}, {a: 2})).toEqual(3);
                });

                it('should expose traverse locals', function () {
                    expect(ngParser('a.b')({a: {b: 0}}, {a: {b: 1}})).toEqual(1);
                    expect(ngParser('a.b')({a: null}, {a: {b: 1}})).toEqual(1);
                    expect(ngParser('a.b')({a: {b: 0}}, {a: null})).toEqual(undefined);
                });
            });

            describe('literal', function () {
                it('should mark scalar value expressions as literal', function () {
                    expect(ngParser('0').literal).toBe(true);
                    expect(ngParser('"hello"').literal).toBe(true);
                    expect(ngParser('true').literal).toBe(true);
                    expect(ngParser('false').literal).toBe(true);
                    expect(ngParser('null').literal).toBe(true);
                    expect(ngParser('undefined').literal).toBe(true);
                });

                it('should mark array expressions as literal', function () {
                    expect(ngParser('[]').literal).toBe(true);
                    expect(ngParser('[1, 2, 3]').literal).toBe(true);
                    expect(ngParser('[1, identifier]').literal).toBe(true);
                });

                it('should mark object expressions as literal', function () {
                    expect(ngParser('{}').literal).toBe(true);
                    expect(ngParser('{x: 1}').literal).toBe(true);
                    expect(ngParser('{foo: bar}').literal).toBe(true);
                });

                it('should not mark function calls or operator expressions as literal', function () {
                    expect(ngParser('1 + 1').literal).toBe(false);
                    expect(ngParser('call()').literal).toBe(false);
                    expect(ngParser('[].length').literal).toBe(false);
                });
            });

            describe('constant', function () {
                it('should mark scalar value expressions as constant', function () {
                    expect(ngParser('12.3').constant).toBe(true);
                    expect(ngParser('"string"').constant).toBe(true);
                    expect(ngParser('true').constant).toBe(true);
                    expect(ngParser('false').constant).toBe(true);
                    expect(ngParser('null').constant).toBe(true);
                    expect(ngParser('undefined').constant).toBe(true);
                });

                it('should mark arrays as constant if they only contain constant elements', function () {
                    expect(ngParser('[]').constant).toBe(true);
                    expect(ngParser('[1, 2, 3]').constant).toBe(true);
                    expect(ngParser('["string", null]').constant).toBe(true);
                    expect(ngParser('[[]]').constant).toBe(true);
                    expect(ngParser('[1, [2, 3], {4: 5}]').constant).toBe(true);
                });

                it('should not mark arrays as constant if they contain any non-constant elements', function () {
                    expect(ngParser('[foo]').constant).toBe(false);
                    expect(ngParser('[x + 1]').constant).toBe(false);
                    expect(ngParser('[bar[0]]').constant).toBe(false);
                });

                it('should mark complex expressions involving constant values as constant', function () {
                    expect(ngParser('!true').constant).toBe(true);
                    expect(ngParser('1 - 1').constant).toBe(true);
                    expect(ngParser('"foo" + "bar"').constant).toBe(true);
                    expect(ngParser('5 != null').constant).toBe(true);
                    expect(ngParser('{standard: 4/3, wide: 16/9}').constant).toBe(true);
                });

                it('should not mark any expression involving variables or function calls as constant', function () {
                    expect(ngParser('true.toString()').constant).toBe(false);
                    expect(ngParser('foo(1, 2, 3)').constant).toBe(false);
                    expect(ngParser('"name" + id').constant).toBe(false);
                });
            });
        });
    });
